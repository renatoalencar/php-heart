...
expr:
	...
	|	match { $$ = $1; }
	|	expr T_PIPE name
			{ $$ = zend_ast_create(ZEND_AST_CALL, $3, zend_ast_create_list(1, ZEND_AST_ARG_LIST, $1)); }
	|	expr T_PIPE variable
			{ if ($3->kind == ZEND_AST_CALL) {
				$3->child[1] = zend_ast_list_add($3->child[1], $1);
				$$ = $3;
			  } else
				$$ = zend_ast_create(ZEND_AST_CALL, $3, zend_ast_create_list(1, ZEND_AST_ARG_LIST, $1));
			}
	|	expr T_PIPE '(' expr ')'
			{ $$ = zend_ast_create(ZEND_AST_CALL, $4, zend_ast_create_list(1, ZEND_AST_ARG_LIST, $1)); }
;
